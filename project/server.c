...
void add_update_msg_lst(struct UPDATE_MSG_NODE *head, int server_index, int update_index);

struct EMAIL_MSG_NODE *find_user_email_head(char *user_name, struct USER_NODE *user_list_head);
struct EMAIL_MSG_NODE *find_user_email_node(struct EMAIL_MSG_NODE *user_email_head, int email_index, int email_server_index);
struct EMAIL_MSG_NODE *find_email(struct USER_NODE *user_list_head, char *user_name,
                                  int email_index, int email_server_index);


struct UPDATE_MSG create_update_msg(char type, int time_stamp, int lamport_index,
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    update_msg_head[i]->update_msg.update_index = update_msg_tail[i]->update_msg.update_index = -1;
  }
  ...
  while (1) {
    ...
    if (src.type == CLIENT) {
      ...
      if (client_msg.type == PRIVATE_GROUP_REQ) {
        ...
        sprintf(file_name, "s%d_%s_s%d_%d", atoi(server_index), email_msg.email.to,
      ...
      /**
         printf("Index_matrix after client operation:\n");
      ...
      if (update_msg.type == EXCHANGE_INDEX_MATRIX) {
        ...
        for (int i = 0; i < 5; i++) {
          if (index_matrix[local_server_index][i] >
            ...
            for (int j =  incoming_matrix[incoming_server_index][i]+1;
                 j <= index_matrix[local_server_index][i]; j ++) {
              // get update message at [local_server_index][i]
              add_update_msg_lst(update_msg_head, i, j);
            }
        ...
        if (check_time_index(atoi(server_index), update_msg.server_index,
          ...
          printf("Received new email update from server %d\n", update_msg.server_index);
          struct EMAIL_MSG email_msg;
        ...
        printf("Received read update from %d.\n", update_msg.server_index);
        /* check */
        if (check_time_index(atoi(server_index), update_msg.server_index,
                             update_msg.update_index, index_matrix)) {

          printf("Begin read mail update.\n");
          printf("username: %s, email_index: %d, email server index: %d.\n", update_msg.user_name, update_msg.email_index, update_msg.email_server_index);
...
void print_email_list(struct EMAIL_MSG_NODE head) {
  ...
  while (cur) {
    printf("Mail (read:%d) with subject: %s, content %s, email_index %d, from server index: %d. \n", cur->email_msg.email.read, cur->email_msg.email.subject, cur->email_msg.email.content, cur->email_msg.email_index, cur->email_msg.server_index);
    cur = cur->next;
  }
}
...
void print_update_msg_list() {
  ...
  for (int i = 0; i < 5; i ++) {
    ...
    while (temp_msg) {
      ...
      printf("email server index: %d ", temp_msg->update_msg.email_server_index);
      printf("\n");
...
struct EMAIL_MSG_NODE *find_email(struct USER_NODE *user_list_head, char *user_name,
                                  int email_index, int email_server_index) {
  struct EMAIL_MSG_NODE *user_email_head = find_user_email_head(user_name, user_list_head);
  if (user_email_head)
    return find_user_email_node(user_email_head, email_index, email_server_index);
...
void add_update_msg_lst(struct UPDATE_MSG_NODE *head, int server_index, int update_index) {
  ...
  while(node) {
    if (node->update_msg.update_index == update_index) {
      ...
      while(pre->next && pre->next->update_msg.time_stamp < node->update_msg.time_stamp ){
        pre = pre->next;
      }
